...
const WhatsAppClientBotDatabase = require('./database');
...
// Клиентский WhatsApp бот работает независимо, с интеграцией в базу данных

// Создание клиентского WhatsApp бота
const clientBot = new Client({
  authStrategy: new LocalAuth({
    clientId: "whatsapp-client-bot"
  }),
  puppeteer: {
    headless: true,
...
// Меню для вопросов
const questionMenu = `
${EMOJI.CALL} Позвонить
${EMOJI.WHATSAPP} WhatsApp
${EMOJI.EMAIL} Email
${EMOJI.BACK} Назад в меню
`;
...
clientBot.on('message', async (message) => {
  ...
  // Проверяем права администратора (можно настроить список номеров)
  const adminPhones = ['77771234567']; // Ваш номер телефона
  
  if (text.includes('/stats') && adminPhones.includes(contact.number)) {
    try {
      const stats = await WhatsAppClientBotDatabase.getBotStats();
      const recentActivity = await WhatsAppClientBotDatabase.getRecentActivity(5);

      const statsMessage = `
📊 *Статистика WhatsApp клиентского бота*
...
clientBot.on('message', async (message) => {
  ...
  if (text.includes('/start') || text.includes('привет') || text.includes('начать')) {
    ...
    const dbUser = await WhatsAppClientBotDatabase.createOrUpdateUser(userInfo);
...
${EMOJI.TERM} Пример: 24
        ...
        const user = userData.get(userId);
        if (user?.dbId) {
          await WhatsAppClientBotDatabase.saveCalculation(
            user.dbId, 
            amount, 
            term, 
            interestRate, 
...
${WEBSITE_URL}/#credit-calculator
      ...
      // Если пользователь в режиме задания вопросов
        ...
        if (fixedAnswer) {
          ...
          if (user?.dbId) {
            await WhatsAppClientBotDatabase.saveQuestion(user.dbId, text, fixedAnswer, true);
          }
          ...
          if (user?.dbId) {
            await WhatsAppClientBotDatabase.saveQuestion(user.dbId, text, null, false);
          }
...
// Обработка QR кода
clientBot.on('qr', (qr) => {
  console.log('📱 QR код для подключения WhatsApp клиент-бота:');
  qrcode.generate(qr, { small: true });
});

// Обработка готовности клиента
clientBot.on('ready', () => {
  console.log('✅ WhatsApp клиентский бот готов к работе!');
  console.log('📱 Номер:', clientBot.info.wid.user);
  console.log('🌐 Сайт:', WEBSITE_URL);
});

// Обработка аутентификации
clientBot.on('authenticated', () => {
  console.log('🔐 WhatsApp клиентский бот аутентификация успешна');
});

// Обработка ошибок
clientBot.on('auth_failure', (msg) => {
  console.error('❌ Ошибка аутентификации WhatsApp клиент-бота:', msg);
});

clientBot.on('disconnected', (reason) => {
  console.log('❌ WhatsApp клиентский бот отключен:', reason);
});

// Проверка подключения к БД при запуске
WhatsAppClientBotDatabase.testConnection().then((connected) => {
  if (connected) {
    ...
    console.log('🤖 WhatsApp клиентский бот инициализируется...');
    ...
    // Инициализируем клиент даже без БД
    clientBot.initialize();
  }
});
